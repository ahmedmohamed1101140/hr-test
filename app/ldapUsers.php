...
class ldapUsers {
    ...
    */
        ...
        if (!array_key_exists("user_name",$attributes)){ return ("Missing compulsory field [username]"); }
        if (!array_key_exists("first_name",$attributes)){ return ("Missing compulsory field [firstname]"); }
        if (!array_key_exists("last_name",$attributes)){ return ("Missing compulsory field [surname]"); }
    ...
    */
	...
	public function user_info_disabled($username,$fields=NULL,$isGUID=false){
        if ($username===NULL){ return (false); }
        if (!$this->_bind){ return (false); }
        if ($isGUID === true) {
            $username = $this->strguid2hex($username);
    ...
    */
	...
	public function folder_list_new($folder_name = NULL, $dn_type = ADLDAP_FOLDER, $recursive = NULL, $type = NULL) {
		if ($recursive===NULL){ $recursive=$this->_recursive_groups; } //use the default option if they haven't set it
		if (!$this->_bind){ return (false); }
		$filter = '(&';
		if ($type !== NULL) {
